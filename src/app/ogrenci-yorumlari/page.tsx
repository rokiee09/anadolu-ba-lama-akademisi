        ...
        <div className="grid gap-6 md:grid-cols-3">
          {testimonials.map((item) => (
            <article key={item.name} className="card-surface p-8">
              ...
              <h2 className="mt-4 text-xl font-semibold text-[#2B1B12]">{item.name}</h2>
              <p className="mt-1 text-sm uppercase tracking-[0.2em] text-[#7A665A]">{item.level}</p>
              <p className="mt-4 leading-7 text-[#5D4538]">{item.quote}</p>
            </article>
          ))}
        </div>
      </section>
